...
import javax.swing.JOptionPane;  //Serve para caixas de diálogo
...
//Laço FOR que imprime na tela de n=1 até n=10 usando o valor recebido pelo usuario de X
  for (int n = 1; n <= 10; n++) {
    double result = aproximaIntegral(n, x, 2.0, 1000000); //Valor de n e depois o de x
    result = (2*result)/(x*i0);  //Como está no livro, multiplica por 2 e divide por I0(x)
    textSize(28);
    text("2*I" + n + "(x)/x*I0(x) = " + result, 10, 150 + n * 35);
  }
}
...
void draw() {
 textSize(32);
  ...
  textSize(22);
  text("Comparar com a Tabela 4.5-2 da página 111", 10, 650);
  println(i0);
}

//FUNÇÃO DO APACHE COMMONS QUE CALCULA A INTEGRAL-----------------
